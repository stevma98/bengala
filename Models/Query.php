...
class Query {
    ...
    public function editQuery1($data){
        try { 
			...
			unset($data['controller'], $data['method'],$data['id'],$data['PHPSESSID']);
            ...
            $user=$_SESSION['user']->identyUser;
            $ide=$_SESSION['user']->ID_EMPRESA;
            $carrito = ['ID_MASCOTA' => $idmascota,'ID_PROP' => $idprop, 'ID_EMPRESA' => $ide,'FECHA_ANADIDO'=>$date,'ID_USUARIO'=>$user,'TIPO'=>'Consulta','ID_PRODUCTO'=>$id,'ESTADO_CARRITO'=>'Pendiente','ID_CONSE_CARRITO'=>$consecutivo];
